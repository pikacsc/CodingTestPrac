...
// A : 왼쪽 카드 더미
// B : 오른쪽 카드 더미
// DP[i][j] : 왼쪽 카드뭉치의 카드가 i개 남고, 오른쪽 카드가 j장 남았을 때, 얻을 수 있는 최고 점수
...
int solution(int left, int right)
{
	...
	val = A[left] > B[right] ? std::max(val, solution(left, right + 1) + B[right]) : std::max(solution(left + 1, right), solution(left + 1, right + 1));
	return val;
}
